...
emit_base_cross_sysroot_deps() {
  ...
  local dep
  for dep in "${base_deps[@]}"; do
    printf "cross_sysroot_base%srequired%s%s%sdpkg -s %s%s\n" \
      "${CROSS_DEP_FIELD_SEP}" \
      "${CROSS_DEP_FIELD_SEP}" "${dep}" \
      "${CROSS_DEP_FIELD_SEP}" "${dep}" \
      "${CROSS_DEP_FIELD_SEP}"
...
cross_dependency_scope_override() {
  local pkg_key="$1"
  local dep_name="$2"

  case "${pkg_key}:${dep_name}" in
    components/model_zoo/vad:python3-pip)
      echo "host"
      ;;
    application/native/reachy_mini:protobuf-compiler|application/native/reachy_mini:protobuf-compiler-grpc)
      echo "host"
      ;;
  esac
}

emit_cross_host_build_dep() {
  local pkg_key="$1"
  local dep_name="$2"
  local check_cmd="$3"

  printf "%s%srequired%s%s%s%s%s%shost\n" \
    "${pkg_key}" "${CROSS_DEP_FIELD_SEP}" \
    "${CROSS_DEP_FIELD_SEP}" "${dep_name}" \
    "${CROSS_DEP_FIELD_SEP}" "${check_cmd}" \
    "${CROSS_DEP_FIELD_SEP}" "${CROSS_DEP_FIELD_SEP}"
}

emit_cross_package_build_deps() {
  local pkg_key="$1"

  case "${pkg_key}" in
    components/model_zoo/vad)
      emit_cross_host_build_dep "${pkg_key}" "python3-setuptools" "python3 -c \"import setuptools\""
      emit_cross_host_build_dep "${pkg_key}" "python3-wheel" "python3 -c \"import wheel\""
      ;;
  esac
}
...
collect_cross_dependency_lines() {
  ...
  case "${cmd}" in
    all|cmake|C|ros2|R)
      ...
      if target_uses_ros2_for_command "${cmd}" && [[ -f "${ros2_deps_xml}" ]]; then
        read_cross_xml_sysdeps "build_ros2_deps" "${ros2_deps_xml}"
      fi
      ...
      for pkg_path in "${enabled_all[@]}"; do
        ...
        emit_cross_package_build_deps "${pkg_path}"
      done
      ;;
    package|pkg)
      local pkg_arg="${1:-}"
      ...
      local ros2_deps_xml="${REPO_ROOT}/build/package_ros2.xml"
      if package_is_ros2 "${pkg_arg}" && [[ -f "${ros2_deps_xml}" ]]; then
        read_cross_xml_sysdeps "build_ros2_deps" "${ros2_deps_xml}"
      fi
      ...
      emit_cross_package_build_deps "${pkg_key}"
      ;;
...
split_cross_dependencies() {
  ...
  while IFS= read -r dep_line; do
    ...
    [[ -n "${dep_name}" ]] || continue
    if [[ -z "${cross_scope}" ]]; then
      cross_scope="$(cross_dependency_scope_override "${pkg_key}" "${dep_name}")"
    fi
...
cross_prepare_bianbu_sysroot_dependency_install() {
  local container_name="$1"
  shift || true
  local dep
  ...
  for dep in "$@"; do
    case "${dep}" in
      spacemit-tcm|spacemit-onnxruntime|python3-spacemit-ort|llama.cpp-tools-spacemit)
        docker exec -i --workdir / --user "0:0" "${container_name}" bash -lc '
          set -euo pipefail
          mkdir -p /dev/shm
          : > /dev/shm/tcm_sync_standalone
          chown root:root /dev/shm/tcm_sync_standalone 2>/dev/null || true
          chmod 666 /dev/shm/tcm_sync_standalone 2>/dev/null || true
        '
        return 0
        ;;
    esac
...
install_deps_in_container() {
  ...
  if [[ "${label}" == "Bianbu sysroot" ]]; then
    cross_prepare_bianbu_sysroot_dependency_install "${container_name}" "${missing_required[@]}"
  fi
  docker exec -i --workdir "${REPO_ROOT}" --user "0:0" "${container_name}" \
...
cross_sysroot_deps_fingerprint() {
  if [[ "${#SYSROOT_DEP_LINES[@]}" -eq 0 ]]; then
    printf "empty\n" | cross_hash_stdin
    return 0
  fi

  printf "%s\n" "${SYSROOT_DEP_LINES[@]}" | LC_ALL=C sort -u | cross_hash_stdin
}

cross_sysroot_success_stamp_file() {
  printf "%s\n" "${CROSS_SYSROOT}/.srobotis_cross_sysroot.success"
}

cross_sysroot_deps_stamp_file() {
  printf "%s\n" "${CROSS_SYSROOT}/.srobotis_cross_sysroot_deps.sha256"
}

cross_sysroot_ready() {
  [[ -d "${CROSS_SYSROOT}/usr" && -f "$(cross_sysroot_success_stamp_file)" ]]
}

cross_sysroot_deps_stamp_matches() {
  local expected="$1"
  local stamp_file
  stamp_file="$(cross_sysroot_deps_stamp_file)"

  [[ -f "${stamp_file}" ]] || return 1
  [[ "$(cat "${stamp_file}")" == "${expected}" ]]
}

write_cross_sysroot_success_stamp() {
  local stamp_file
  stamp_file="$(cross_sysroot_success_stamp_file)"

  [[ -d "${CROSS_SYSROOT}" ]] || return 0
  printf "success %s\n" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" > "${stamp_file}"
}

write_cross_sysroot_deps_stamp() {
  local fingerprint="$1"
  local stamp_file
  stamp_file="$(cross_sysroot_deps_stamp_file)"
...
sync_bianbu_sysroot() {
  ...
  if [[ "${SROBOTIS_CROSS_REFRESH_SYSROOT:-0}" != "1" && "${deps_changed}" != "1" ]] && cross_sysroot_ready; then
    normalize_cross_sysroot "${CROSS_SYSROOT}"
    echo "[cross] Reusing existing sysroot: ${CROSS_SYSROOT}"
    return 0
  fi
...
cross_cmake_extra_args() {
  local bianbu_tag="${CROSS_BIANBU_TAG:-4.0}"
  ...
  printf '%s\n' \
    ...
    "-DCMAKE_EXE_LINKER_FLAGS=-Wl,-rpath-link,${CROSS_STAGING_PREFIX}/lib -Wl,-rpath-link,${CROSS_SYSROOT}/usr/lib/riscv64-linux-gnu -Wl,-rpath-link,${CROSS_SYSROOT}/usr/lib" \
    ...
    "-DRUST_TARGET_DIR=${DEFAULT_OUTPUT_ROOT}/cross/.cache/${bianbu_tag}/cargo/xl330"
...
cross_motor_uart_xl330_requested() {
  local pkg_key="components/peripherals/motor"

  if [[ -z "${BUILD_CONFIG_FILE:-}" || ! -f "${BUILD_CONFIG_FILE}" ]] || ! has_jq; then
    return 0
  fi

  local drivers=()
  mapfile -t drivers < <(get_target_option_list "${pkg_key}" ".enabled_drivers[]? // empty")

  # The motor CMake treats an empty driver list as auto-detect, which includes
  # drv_uart_xl330 when the driver source exists.
...
cross_should_prebuild_motor_uart_xl330() {
  local cmd="$1"
  shift || true
  ...
  case "${cmd}" in
    all|cmake|C)
      ...
      while IFS= read -r pkg_path; do
        [[ "${pkg_path}" == "components/peripherals/motor" ]] || continue
        cross_motor_uart_xl330_requested
        return $?
    ...
    package|pkg)
      ...
      [[ -n "${pkg_arg}" ]] || return 1
      [[ "${action}" == "clean" ]] && return 1

      local pkg_key
      pkg_key="$(package_key_from_arg "${pkg_arg}")"
      [[ "${pkg_key}" == "components/peripherals/motor" ]] || return 1
      cross_motor_uart_xl330_requested
      return $?
      ;;
  esac
...
prebuild_motor_uart_xl330_rust_in_bianbu() {
  local container_name="$1"
  local rust_src="${REPO_ROOT}/components/peripherals/motor/src/drivers/drv_uart_xl330"
  local cross_cache_root="${DEFAULT_OUTPUT_ROOT}/cross/.cache"
  local rust_tag_cache_dir="${cross_cache_root}/${CROSS_BIANBU_TAG}"
  local cargo_cache_dir="${rust_tag_cache_dir}/cargo"
  local rust_target_dir="${cargo_cache_dir}/xl330"
  local cargo_home="${CROSS_OUTPUT_ROOT}/.cargo"
  local cargo_user_home="${CROSS_OUTPUT_ROOT}/.cargo_home"
  local uid gid
  uid="$(id -u)"
  gid="$(id -g)"
  ...
  echo "[cross] Prebuilding motor UART XL330 Rust core in Bianbu container..."
  docker exec -i --workdir / --user "0:0" "${container_name}" \
    bash -c "set -euo pipefail; uid=\"\$1\"; gid=\"\$2\"; cross_cache_root=\"\$3\"; rust_tag_cache_dir=\"\$4\"; cargo_cache_dir=\"\$5\"; shift 5; mkdir -p \"\${cross_cache_root}\" \"\${rust_tag_cache_dir}\" \"\${cargo_cache_dir}\"; chown \"\${uid}:\${gid}\" \"\${cross_cache_root}\" \"\${rust_tag_cache_dir}\" \"\${cargo_cache_dir}\"; for path in \"\$@\"; do mkdir -p \"\${path}\"; chown -R \"\${uid}:\${gid}\" \"\${path}\"; done" \
    bash "${uid}" "${gid}" "${cross_cache_root}" "${rust_tag_cache_dir}" "${cargo_cache_dir}" "${cargo_home}" "${rust_target_dir}" "${cargo_user_home}"

  local cargo_rc=0
  docker exec -i \
    --workdir "${rust_src}" \
    --user "${uid}:${gid}" \
    -e "HOME=${cargo_user_home}" \
    -e "CARGO_HOME=${cargo_home}" \
    -e "CARGO_TARGET_DIR=${rust_target_dir}" \
    "${container_name}" \
    bash -s <<'CARGO_EOF' || cargo_rc=$?
set -euo pipefail
mkdir -p "${CARGO_HOME}" "${CARGO_TARGET_DIR}"
CARGO_INCREMENTAL=1 cargo build --release --lib --target-dir "${CARGO_TARGET_DIR}"
touch "${CARGO_TARGET_DIR}/.build_stamp"
CARGO_EOF
  docker exec -i --workdir / --user "0:0" "${container_name}" \
    bash -c "set -euo pipefail; uid=\"\$1\"; gid=\"\$2\"; cross_cache_root=\"\$3\"; rust_tag_cache_dir=\"\$4\"; cargo_cache_dir=\"\$5\"; shift 5; for path in \"\${cross_cache_root}\" \"\${rust_tag_cache_dir}\" \"\${cargo_cache_dir}\"; do if [[ -e \"\${path}\" ]]; then chown \"\${uid}:\${gid}\" \"\${path}\" 2>/dev/null || true; fi; done; for path in \"\$@\"; do if [[ -e \"\${path}\" ]]; then chown -R \"\${uid}:\${gid}\" \"\${path}\" 2>/dev/null || true; fi; done" \
    bash "${uid}" "${gid}" "${cross_cache_root}" "${rust_tag_cache_dir}" "${cargo_cache_dir}" "${cargo_home}" "${rust_target_dir}" "${cargo_user_home}" || true
  return "${cargo_rc}"
}
...
run_build_in_ubuntu() {
  ...
  docker exec -i \
    ...
    -e "SROBOTIS_OUTPUT_ROOT=${CROSS_OUTPUT_ROOT}" \
    -e "SROBOTIS_OUTPUT_STAGING=${CROSS_STAGING_PREFIX}" \
    -e "SROBOTIS_OUTPUT_ROOTFS=${CROSS_ROOTFS_PREFIX}" \
    -e "SROBOTIS_OUTPUT=${CROSS_STAGING_PREFIX}" \
...
relocate_cross_text_file_replace() {
  local file="$1"
  local from="$2"
  local to="$3"

  grep -Iq . "${file}" 2>/dev/null || return 1
  grep -qF "${from}" "${file}" 2>/dev/null || return 1
...
relocate_cross_ros2_install() {
  ...
  local file
  while IFS= read -r -d "" file; do
    if relocate_cross_text_file_replace "${file}" "${build_ros_prefix}" "${runtime_ros_prefix}"; then
      rewritten=1
    fi
    if [[ -n "${build_staging_prefix}" && "${build_staging_prefix}" != "${install_prefix}" ]] && \
      relocate_cross_text_file_replace "${file}" "${build_staging_prefix}" "${install_prefix}"; then
      rewritten=1
    fi
  done < <(
...
main() {
  ...
  local sysroot_deps_fingerprint
  sysroot_deps_fingerprint="$(cross_sysroot_deps_fingerprint)"
  if [[ "${SROBOTIS_CROSS_SKIP_SYSROOT_SYNC:-0}" != "1" ]] && \
    ! cross_sysroot_deps_stamp_matches "${sysroot_deps_fingerprint}"; then
    echo "[cross] Sysroot dependency set changed; refreshing sysroot"
    sysroot_deps_changed=1
  fi

  sync_bianbu_sysroot "${bianbu_container}" "${sysroot_deps_changed}"
  write_cross_sysroot_deps_stamp "${sysroot_deps_fingerprint}"
  write_toolchain_file

  if cross_should_prebuild_motor_uart_xl330 "${cmd}" "${payload[@]}"; then
    prebuild_motor_uart_xl330_rust_in_bianbu "${bianbu_container}"
  fi
