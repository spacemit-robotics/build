...
def test_cross_build_routes_application_sysdeps_to_sysroot_unless_host_scoped(tmp_path: Path) -> None:
    ...
    result = run_cmd(
        ...
        "echo HOST:\\${HOST_DEP_LINES[*]}; "
        "echo SYSROOT:\\${SYSROOT_DEP_LINES[*]}\"",
    )
...
def test_cross_build_prebuilds_motor_xl330_rust_in_bianbu(tmp_path: Path) -> None:
    sdk = make_sdk(tmp_path)
    write_package(
        sdk,
        "components/peripherals/motor",
        name="motor",
        sysdeps=[
            ("libboost-dev", "dpkg -s libboost-dev", None),
            ("libgpiod-dev", "pkg-config --exists libgpiod", None),
    ...
    write_file(
        ...
        """
        {
          ...
          "enabled_package_options": {
            "components/peripherals/motor": { "enabled_drivers": ["drv_uart_xl330"] }
          }
        }
        """,
    )
    write_file(
        sdk / "target" / "k3-can.json",
        """
        {
          "version": "1.0",
          "board": "k3-com260",
    ...
    result = run_cmd(
        sdk,
        "SROBOTIS_CROSS_BUILD_SH_NO_MAIN=1 BUILD_TARGET=k3-xl330 "
        "bash -c \"source build/cross_build.sh; load_build_config >/dev/null; "
        "split_cross_dependencies package components/peripherals/motor; "
        "echo HOST:\\${HOST_DEP_LINES[*]}; "
        "echo SYSROOT:\\${SYSROOT_DEP_LINES[*]}; "
        "if cross_should_prebuild_motor_uart_xl330 package components/peripherals/motor build; "
        "then echo PREBUILD:yes; else echo PREBUILD:no; fi\"",
    )

    assert result.returncode == 0, result.stdout
    host_line = next(line for line in result.stdout.splitlines() if line.startswith("HOST:"))
    sysroot_line = next(line for line in result.stdout.splitlines() if line.startswith("SYSROOT:"))
    assert "rustc" not in host_line
    assert "cargo" not in host_line
    assert "rustc" in sysroot_line
    assert "cargo" in sysroot_line
    assert "PREBUILD:yes" in result.stdout

    result = run_cmd(
        sdk,
        "SROBOTIS_CROSS_BUILD_SH_NO_MAIN=1 BUILD_TARGET=k3-can "
        "bash -c \"source build/cross_build.sh; load_build_config >/dev/null; "
        "if cross_should_prebuild_motor_uart_xl330 package components/peripherals/motor build; "
        "then echo PREBUILD:yes; else echo PREBUILD:no; fi\"",
    )
    ...
    assert result.returncode == 0, result.stdout
    assert "PREBUILD:no" in result.stdout


def test_cross_build_routes_vad_python_wheel_tools_to_host(tmp_path: Path) -> None:
    sdk = make_sdk(tmp_path)
    write_package(
        sdk,
        ...
        name="vad",
        sysdeps=[
            ("spacemit-onnxruntime", "dpkg -s spacemit-onnxruntime", "riscv64"),
            ("python3-spacemit-ort", "dpkg -s python3-spacemit-ort", "riscv64"),
            ("python3-pip", None, None),
        ],
        cmake=True,
    )
    write_file(
        sdk / "target" / "k3-vad.json",
        """
        {
          "version": "1.0",
          "board": "k3-com260",
          "enabled_packages": ["components/model_zoo/vad"]
        }
        """,
    )
    ...
    result = run_cmd(
        sdk,
        "SROBOTIS_CROSS_BUILD_SH_NO_MAIN=1 BUILD_TARGET=k3-vad "
        "bash -c \"source build/cross_build.sh; load_build_config >/dev/null; "
        "split_cross_dependencies all; "
        "echo HOST:\\${HOST_DEP_LINES[*]}; "
        "echo SYSROOT:\\${SYSROOT_DEP_LINES[*]}\"",
    ...
    host_line = next(line for line in result.stdout.splitlines() if line.startswith("HOST:"))
    sysroot_line = next(line for line in result.stdout.splitlines() if line.startswith("SYSROOT:"))
    assert "python3-pip" in host_line
    assert "python3-setuptools" in host_line
    assert "python3-wheel" in host_line
    assert "spacemit-onnxruntime" in sysroot_line
    assert "python3-spacemit-ort" in sysroot_line
    assert "python3-pip" not in sysroot_line


def test_cross_build_routes_reachy_mini_protobuf_generators_to_host(tmp_path: Path) -> None:
    sdk = make_sdk(tmp_path)
    write_package(
        sdk,
        ...
        name="reachy_mini",
        sysdeps=[
            ("libprotobuf-dev", "pkg-config --exists protobuf", None),
            ("protobuf-compiler", "which protoc", None),
            ("libgrpc++-dev", "pkg-config --exists grpc++", None),
            ("protobuf-compiler-grpc", "which grpc_cpp_plugin", None),
        ],
        cmake=True,
    )
    write_file(
    ...
    result = run_cmd(
        sdk,
        "SROBOTIS_CROSS_BUILD_SH_NO_MAIN=1 BUILD_TARGET=k3-reachy "
        "bash -c \"source build/cross_build.sh; load_build_config >/dev/null; "
        "split_cross_dependencies all; "
        "echo HOST:\\${HOST_DEP_LINES[*]}; "
        "echo SYSROOT:\\${SYSROOT_DEP_LINES[*]}\"",
    ...
    host_line = next(line for line in result.stdout.splitlines() if line.startswith("HOST:"))
    sysroot_line = next(line for line in result.stdout.splitlines() if line.startswith("SYSROOT:"))
    assert "protobuf-compiler" in host_line
    assert "protobuf-compiler-grpc" in host_line
    assert "libprotobuf-dev" not in host_line
    assert "libgrpc++-dev" not in host_line
    assert "libprotobuf-dev" in sysroot_line
    assert "libgrpc++-dev" in sysroot_line
    assert "protobuf-compiler" not in sysroot_line
    assert "protobuf-compiler-grpc" not in sysroot_line
...
def test_cross_build_merges_user_cmake_extra_args(tmp_path: Path) -> None:
    ...
    assert "-DGIT_EXECUTABLE=/usr/bin/git" in lines
    assert "-DPROTOC_EXE=/usr/bin/protoc" in lines
    assert "-DGRPC_CPP_PLUGIN_EXE=/usr/bin/grpc_cpp_plugin" in lines
    assert "-DCMAKE_FIND_ROOT_PATH_MODE_PROGRAM=NEVER" in lines
